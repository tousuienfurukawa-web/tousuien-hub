...
def fetch_excel_from_github(path: str = Query(...)):
    ...
    # Excelを読み取り専用で開く（Vercel向け）
    workbook = openpyxl.load_workbook(io.BytesIO(res.content), data_only=True, read_only=True, keep_links=False)
    ...
    return json.dumps(summary[:10], ensure_ascii=False, indent=2)
